...
-opaque next(MFR, MFS) :: {continue,
        MFR,
        gleam@option:option(gleam@erlang@process:selector(MFS))} |
    normal_stop |
...
-opaque builder(MFT, MFU) :: {builder,
        integer(),
        fun((gleam@http@request:request(MFT)) -> gleam@http@response:response(MFU)),
...
-type websocket_message(MFV) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, MFV}.
...
-type chunk_next(MFW) :: {chunk_continue, MFW} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(MFX) -> next(MFX, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(MGB, MGC), gleam@erlang@process:selector(MGC)) -> next(MGB, MGC).
...
-spec convert_next(next(MGQ, MGR)) -> mist@internal@next:next(MGQ, MGR).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(MHS)) -> gleam@http@response:response(MHU))
) -> builder(MHS, MHU).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(MHY, MHZ), integer()) -> builder(MHY, MHZ).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), MIE),
    integer(),
    gleam@http@response:response(MIE)
) -> builder(mist@internal@http:connection(), MIE).
...
-spec bind(builder(MIQ, MIR), binary()) -> builder(MIQ, MIR).
...
-spec with_ipv6(builder(MIW, MIX)) -> builder(MIW, MIX).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(MJC, MJD), binary(), binary()) -> builder(MJC, MJD).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(MJS)
) -> {ok, websocket_message(MJS)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((MJY, websocket_message(MJZ), mist@internal@websocket:websocket_connection()) -> next(MJY, MJZ)),
    fun((mist@internal@websocket:websocket_connection()) -> {MJY,
        gleam@option:option(gleam@erlang@process:selector(MJZ))}),
    fun((MJY) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(MKN)) -> MKP),
    fun((MKP, MKN, s_s_e_connection()) -> gleam@otp@actor:next(MKP, MKN))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(MLF) -> chunk_next(MLF).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(MKY)) -> MLA),
    fun((MLA, MKY, mist@internal@http:connection()) -> chunk_next(MLA))
) -> gleam@http@response:response(response_data()).
