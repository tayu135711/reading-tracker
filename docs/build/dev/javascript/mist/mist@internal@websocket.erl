...
-type valid_message(LRO) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, LRO}.

-type websocket_message(LRP) :: {valid, valid_message(LRP)} | invalid.
...
-type handler_message(LRQ) :: {internal, gramps@websocket:frame()} | {user, LRQ}.

-type websocket_state(LRR) :: {websocket_state,
        bitstring(),
        LRR,
...
-file("src/mist/internal/websocket.gleam", 384).
?DOC(false).
...
-file("src/mist/internal/websocket.gleam", 391).
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(LSV))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(LSV))).
...
-spec apply_frames(
    ...
    fun((LSL, handler_message(LSM), websocket_connection()) -> mist@internal@next:next(LSL, LSM)),
    websocket_connection(),
    mist@internal@next:next(LSL, websocket_message(LSM)),
    fun((LSL) -> nil)
) -> mist@internal@next:next(LSL, websocket_message(LSM)).
            ...
            case exception_ffi:rescue(
                ...
                {ok, {abnormal_stop, Reason@2}} ->
                    _ = glisten@transport:send(
                        erlang:element(3, Connection),
                        erlang:element(2, Connection),
                        gramps@websocket:encode_close_frame(
                            {custom_close_reason,
                                4000,
                                gleam_stdlib:identity(Reason@2)},
                            none
                        )
                    ),
                    On_close(State@2),
...
-spec initialize_connection(
    fun((websocket_connection()) -> {LSA,
        gleam@option:option(gleam@erlang@process:selector(LSB))}),
    fun((LSA) -> nil),
    fun((LSA, handler_message(LSB), websocket_connection()) -> mist@internal@next:next(LSA, LSB)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:pid_())} |
...
initialize_connection(On_init, On_close, Handler, Socket, Transport, Extensions) ->
                ...
                {valid, {user_message, Msg@1}} ->
                    ...
                    _pipe@14 = gleam@result:map(
                        ...
                        fun(Cont) -> case Cont of
                                ...
                                normal_stop ->
                                    _ = glisten@transport:send(
                                        erlang:element(3, Connection@1),
                                        erlang:element(2, Connection@1),
                                        gramps@websocket:encode_close_frame(
                                            {normal, <<>>},
                                            none
                                        )
                                ...
                                {abnormal_stop, Reason@1} ->
                                    _ = glisten@transport:send(
                                        ...
                                        gramps@websocket:encode_close_frame(
                                            {custom_close_reason,
                                                4000,
                                                gleam_stdlib:identity(Reason@1)},
                                            none
                                        )
                                    ),
                                    _ = gleam@option:map(
    ...
    gleam@result:map(
        ...
        fun(Subj) ->
            ...
            {started, Websocket_pid@1, Websocket_pid@1}
        end
    ).
